...
class DynamoElasticSync:
    def __init__(self):
        ...
        self.sqs_client = aioboto3.client("sqs", region_name=region)
    ...
    def _deserialise_image(self, dynamodb_section, image_name):
        if image_name in dynamodb_section:
            return {k: self.deserialiser.deserialize(v) for k, v in dynamodb_section[image_name].items()}
        else:
            return {}
