...
resource "null_resource" "setup_new_index" {
  ...
  provisioner "local-exec" {
    ...
    command = "python ${path.module}/write-new-index.py ${var.aws_region} ${var.app_name} ${var.task_name} ${var.index_name} ${local.index_hash} ${data.local_file.index_definition.filename} ${data.aws_ssm_parameter.es_domain.value} ${local.old_index_hash}"
  }
}
