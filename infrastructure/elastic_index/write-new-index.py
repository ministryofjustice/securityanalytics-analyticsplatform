...
if len(sys.argv[1:]) not in range(7, 9):
    raise ValueError(f"write-new-index.py region app_name task_name index_hash index_file url old_index")

region, app_name, task_name, index_name, index_hash, index_file_name, url = sys.argv[1:8]
old_index = sys.argv[8] if len(sys.argv) == 9 else None
...
write_alias = f"{task_name}:{index_name}:write"
read_alias = f"{task_name}:{index_name}:read"
new_index = f"{task_name}:{index_name}:{index_hash}"
